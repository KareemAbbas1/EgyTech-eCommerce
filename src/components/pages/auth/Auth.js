...
const AuthContainer = () => {
  const [visibleForm, show] = useState("reset");
...
const ResetPassword = (props) => {
  const [isDisabled, setDisabled] = useState(true);
  const [emailValue, updateEmail] = useState("");
  const [isEmailNotExist, showInvalidFeedback] = useState(false);
  const [isSubmitting, toggleSubmitting] = useState(false);
  const [feedbackVisibility, toggleFeedback] = useState("none");
  ...
  return (
    ...
    >
      ...
      ></input>
      <div
        className="fieldInvalidFeedback invalid-feedback w-75 mx-auto mt-n2 mb-2 "
        style={{ display: feedbackVisibility }}
      >
        Please, check the email because it's not on our system
      </div>
      <button
        type="submit"
        className="d-block btn btn-primary px-4 py-1 mx-auto border-0 fw-bold text-light"
...
const Feedback = (props) => {
  return (
    <div className="col-11 col-sm-7 col-xl-5 px-2 py-4 border border-primary rounded bg-light bg-opacity-10 text-light">
      <div id="envelopeContainer" className="d-flex justify-content-center">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="120"
          height="120"
      ...
      <h1 className="display-7 text-center mb-3">Check your mail</h1>
      <p className="text-center px-5 mb-5">
        We have sent a password recover insturcutions to your email
      </p>

      <p className="text-center fs-6">
        Did not receive the email? Check your spam folder, or{" "}
        <a href="#" className="text-decoration-none " onClick={props.showReset}>
